...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.nonEmpty) points.groupBy(point => findClosest(point, means))
    else means.map(mean => (mean, List())).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(mean => findAverage(mean, classified.getOrElse(mean, List())))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val pairs = oldMeans.zip(newMeans)
    val distances = pairs.map(pair => pair._1.squareDistance(pair._2))
    distances.forall(_ <= eta)
  }
